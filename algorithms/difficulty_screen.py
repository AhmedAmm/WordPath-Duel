...
def draw_difficulty_screen(screen, COLORS, FONT, WIDTH, HEIGHT, level):
    screen.fill(COLORS['background'])
    title_text = FONT.render("Select Difficulty Level", True, COLORS['text'])
    screen.blit(title_text, (WIDTH // 2 - 150, HEIGHT // 2 - 200))
    
    slider_rect = pygame.Rect(WIDTH // 2 - 150, HEIGHT // 2 - 50, 300, 10)
    pygame.draw.rect(screen, COLORS['text'], slider_rect)
    
    for i in range(1, 6):
        level_pos = WIDTH // 2 - 150 + (i - 1) * 75
        pygame.draw.circle(screen, COLORS['text'], (level_pos, HEIGHT // 2 - 45), 10)
        level_text = FONT.render(f"{i}", True, COLORS['text'])
        screen.blit(level_text, (level_pos - 10, HEIGHT // 2 - 80))
    
    handle_pos = WIDTH // 2 - 150 + (level - 1) * 75
    pygame.draw.circle(screen, COLORS['player'], (handle_pos, HEIGHT // 2 - 45), 15)
    
    grid_size_text = FONT.render(f"Grid Size: {get_grid_size(level)}x{get_grid_size(level)}", True, COLORS['text'])
    screen.blit(grid_size_text, (WIDTH // 2 - 100, HEIGHT // 2 + 20))
    
    go_back_text = FONT.render("Go Back", True, COLORS['text'])
    go_back_button = pygame.Rect(20, 20, 100, 40)
    ...
    confirm_button = pygame.Rect(WIDTH // 2 - 75, HEIGHT // 2 + 80, 150, 40)
